...
class InvitationViewSet(viewsets.GenericViewSet):
    ...
    def get_queryset(self):
        return WorkspaceInvitation.objects.filter(workspace=self.kwargs["workspace_id"])
    ...
    def accept(self, request, pk=None):
        ...
        if invitation.user != request.user:
            raise PermissionDenied("You cannot accept another user's invitation.")
    ...
    def reject(self, request, pk=None):
        invitation = get_object_or_404(WorkspaceInvitation, pk=pk)
        if invitation.user != request.user:
            raise PermissionDenied("You cannot reject another user's invitation.")
